...
function Widgets() {
    ...
    // const storedApiCall = responseData;
    ...
    return (
        <div className="widgets">
            ...
            {JSON.parse(sessionStorage.getItem('news')).slice(0, 5).map((articles, index) => {
                return (<div key={index}><a href={articles.url} target="_blank" rel="noopener noreferrer">{newsArticle(articles.title, articles.source)}</a></div>)
            })}
        </div>
    )
}
